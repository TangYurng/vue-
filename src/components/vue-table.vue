  ...
  <div id="tableComponent" ref="tableScroll">
    ...
    <div class="content">
      ...
      <div class="table-fixed-left" v-if="showSelect && selectFixed">
        ...
        >
          <table cellpadding="0px" cellspacing="0px" style="width: 50px;">
            ...
            <thead>
              <tr
                :style="{height: titleHeight + 'px', borderBottom: (titleFixed=='auto') ? 0 :
							'1px solid #ddd'}"
              >
                <th>
      ...
      >
          ...
          <table class="scroll-header" :style="{width: setWithd}" ref="cHead">
            ...
            <thead>
              ...
              >
                ...
                <th v-for="(item,index) in tcolumns" :key="index">{{item.title}}</th>
        ...
        <div
          class="table-body"
          :style="titleFixed == 'fixed' ? {paddingTop: titleHeight + 'px'} : ''"
        >
            ...
            <tbody>
              ...
              >
                ...
                <td
                  ...
                  :key="index"
                  :class="'text' + item.textAlign"
                  v-ellipsis="dataItem[item.key] + ',' +item.textLine"
                >{{dataItem[item.key]}}</td>
                <!-- 操作 -->
                <td
                  :class="isDisAbled(index) ? 'textDisable' : ''"
                  v-if="showHandle"
                  class="handleAction"
                >
          ...
          if (width > max) {
            ...
            if (width > max) {
              while (width > max && stop > 1) {
                stop--;
                tempStr = text.substring(0, stop - 5) + ellipsisChar;
                clone.innerHTML = tempStr;
                width = clone.offsetWidth;
              }
            } else if (width < max) {
    ...
    isAllSelect(isSelect) {
      ...
      if (isSelect) {
        this.tdata.forEach((item, index) => {
          if (!item.disable && this.checkedItem.indexOf(index) < 0) {
    ...
    scrollTable(e) {
      ...
      timer = setTimeout(() => {
        if (this.titleFixed === "fixed") {
          ...
          _this.$refs.cHead.style.transform =
          ...
          if (_this.handleFixed) {
            _this.$refs.tFixedRight.style.transform =
              "translateY(" + -e.target.scrollTop + "px)";
          }
        }
      }, 10);
